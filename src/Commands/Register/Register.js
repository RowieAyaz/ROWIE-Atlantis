...
if(!member) return message.channel.send(yanlisEmbed.setDescription(`Bir kullanıcı belirtmelisiniz.`).setFooter(``)).then(m => m.delete({timeout:6500}))
if(!args[1]) return message.channel.send(yanlisEmbed.setDescription(`Bir isim belirtmelisin.`).setFooter(``)).then(m => m.delete({timeout:6500}))
if(!args[2]) return message.channel.send(yanlisEmbed.setDescription(`Bir yaş belirtmelisin.`).setFooter(``)).then(m => m.delete({timeout:6500}))
let Name = args[1].charAt(0).toUpperCase() + args[1].slice(1).toLowerCase();
let Age = Number(args[2])
if(member.id === message.author.id) return message.channel.send(yanlisEmbed.setDescription(`Kendini kayıt edemezsin.`).setFooter(``)).then(m => m.delete({timeout:6500}))
if(member.id === member.manageable) return message.channel.send(yanlisEmbed.setDescription(`Belirtilen kullanıcı üzerinde komut geçersiz.`).setFooter(``)).then(m => m.delete({timeout:6500}))
if(member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send(yanlisEmbed.setDescription(`Kendini kayıt edemezsin.`).setFooter(``)).then(m => m.delete({timeout:6500}))
...
.setFooter(``)).then(m => m.delete({timeout: 7500}))
...
message.channel.send(new MessageEmbed().setDescription(`${member} kullanıcısının ismi "${ServerName}" olarak değiştirildi \nKullanıcıya ${message.author} tarafından <@&${env.Roles.woman_role}> verildi.`).setAuthor(member.user.tag, member.user.avatarURL({dynamic:true})).setColor(env.Colors.pink).setFooter(``))
member.setNickname(`${ServerName}`)
member.roles.add(env.Roles.woman_role)
member.roles.remove(env.Roles.unregister_role)

client.channels.cache.get(env.Channels.register_channel).send(new MessageEmbed().setTitle(`Bir Üye Kayıt Edildi !`).setDescription(`\`•\` Kullanıcı: ${member} | (${member.id})\n\`•\` Yetkili: ${message.author} | (${message.author.id})\n\n\`•\` Yeni İsim: \`${ServerName}\`\n\`•\` Kullanıcının Cinsiyeti: \`Kadın\`\n\`•\` Verilen Rol: <@&${env.Roles.woman_role}>\n\`•\` Kayıt Saati: \`${kayıtsaat}\`\n\`•\` Kayıt Tarihi: \`${moment(Date.now()).format("DD MMMM YYYY")}\``).setThumbnail(member.user.avatarURL({dynamic:true})).setColor(env.Colors.pink).setFooter(``))
...
.setFooter(``)).then(m => m.delete({timeout:6500}))}
