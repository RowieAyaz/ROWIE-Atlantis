...
if(!member) return message.channel.send(yanlisEmbed.setDescription(`Bir kullanıcı etiketle.`).setFooter(``))
UserDB.find({userID: member.id}, async(err, data) => {
if(err) return console.log(err.message)
if(data.length < 2) {message.channel.send(yanlisEmbed.setDescription(`Kullanıcının sadece tek bir \`İsim Yaş\` kayıtı bulundu bundan dolayı liste olarak gösterilmemekte.`).setFooter(``)).then(m => m.delete({timeout:6500}))}
if(data.length > 1) {    
